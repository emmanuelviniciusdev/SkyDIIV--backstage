# apps/worker-outbox-events/observability Specification

## Purpose

Export traces, metrics, and logs from worker-outbox-events to Grafana Cloud over OTLP without changing outbox routing or QStash contracts, and without coupling the processor to Grafana.

## Requirements

### Requirement: Request telemetry is emitted for every fetch

The worker MUST record a root span, request count/error/duration metrics, and structured logs for every HTTP request, including unsigned `GET /` and signed `POST /process-outbox-event`. Resource attributes MUST include `service.name` `worker-outbox-events`. Span attributes MUST include HTTP method, path, and status code. Telemetry attributes MUST NOT include email, clothing images, LLM prompt bodies, or full QStash payloads.
...
#### Scenario: Process-outbox-event POST is recorded

- **GIVEN** a valid QStash signature
- **WHEN** the worker handles `POST /process-outbox-event`
- **THEN** outbox processing is unchanged from today
- **AND** a span is recorded for method `POST`, path `/process-outbox-event`, and the HTTP status returned
...
An OTLP export error MUST be logged and MUST NOT change the HTTP status or outbox side effects (locks, dispatch, status updates).
...
### Requirement: Unsigned process-outbox-event POST still returns 401 and is recorded

QStash signature verification MUST be unchanged. An unsigned or invalid `upstash-signature` on `POST /process-outbox-event` MUST return `401`, MUST NOT dispatch the event, and MUST still record telemetry for the rejected request.

#### Scenario: Unsigned process-outbox-event

- **GIVEN** `POST /process-outbox-event` with no valid `upstash-signature`
- **WHEN** the worker handles the request
- **THEN** the response status is `401`
- **AND** no downstream workflow is invoked
