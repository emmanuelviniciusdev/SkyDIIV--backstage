# apps/worker-ai-workflows/observability Specification

## Purpose

Export traces, metrics, and logs from worker-ai-workflows to Grafana Cloud over OTLP without changing HTTP, QStash, or workflow contracts, and without coupling request handling to Grafana.

## Requirements

### Requirement: Request telemetry is emitted for every fetch

The worker MUST record a root span, request count/error/duration metrics, and structured logs for every HTTP request, including unsigned `GET /` and signed workflow POSTs. Resource attributes MUST include `service.name` `worker-ai-workflows`. Span attributes MUST include HTTP method, path, and status code. Telemetry attributes MUST NOT include email, clothing images, LLM prompt bodies, or full QStash payloads.
...
#### Scenario: Workflow POST is recorded

- **GIVEN** a valid QStash signature
- **WHEN** the worker handles `POST /generate-weekly-outfits`
- **THEN** the workflow response status is unchanged from today
- **AND** a span is recorded for method `POST`, path `/generate-weekly-outfits`, and the HTTP status returned
...
An OTLP export error MUST be logged and MUST NOT change the HTTP status or workflow side effects.
...
### Requirement: Unsigned workflow POST still returns 401 and is recorded

QStash signature verification MUST be unchanged. An unsigned or invalid `upstash-signature` on a workflow POST MUST return `401`, MUST NOT start the workflow, and MUST still record telemetry for the rejected request.

#### Scenario: Unsigned generate-weekly-outfits

- **GIVEN** `POST /generate-weekly-outfits` with no valid `upstash-signature`
