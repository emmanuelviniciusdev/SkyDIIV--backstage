...
resource "oci_budget_budget" "css" {
  ...
  description  = "Hard monthly spend ceiling for robot-shopping-suggestions (${var.cost_limit_usd} USD)"
...
resource "oci_budget_alert_rule" "actual_at_limit" {
  ...
  threshold      = 100
  message        = "SkyDIIV robot-shopping-suggestions: monthly cost limit (${var.cost_limit_usd} USD) reached. Cost guard will terraform-destroy the application infrastructure."
...
resource "oci_budget_alert_rule" "actual_warning" {
  ...
  threshold_type = "PERCENTAGE"
  threshold      = 80
  message        = "SkyDIIV robot-shopping-suggestions: 80% of monthly cost limit (${var.cost_limit_usd} USD) consumed."
  recipients     = var.cost_alert_email
