...
import { createObservabilityProvider } from "../../src/infrastructure/observability/observability.factory"
import { GrafanaCloudOtlpProvider } from "../../src/infrastructure/observability/providers/grafana-cloud.otlp.provider"
import { NoopObservabilityProvider } from "../../src/infrastructure/observability/providers/noop.provider"
...
function otlpInput() {
  return {
    serviceName: "worker-outbox-events",
...
describe("NoopObservabilityProvider", () => {
  it("never calls fetch on flush", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock)
    const port = new NoopObservabilityProvider("worker-outbox-events")
    port.startSpan("HTTP GET").end()
