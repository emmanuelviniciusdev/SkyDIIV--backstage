...
describe("workflows registry", () => {
  it("exposes the process-outbox-event endpoint key", () => {
    expect(Object.keys(workflowRegistry)).toContain("process-outbox-event")
  })
...
describe("workflowsFetch", () => {
  it("passes WORKER_OUTBOX_EVENTS_URL as serveMany baseUrl", async () => {
    const request = new Request("https://worker-outbox-events.workers.dev/process-outbox-event", {
      method: "POST",
    })
    const env = {
    ...
    expect(state.mockServeMany).toHaveBeenCalledWith(workflowRegistry, {
      baseUrl: "https://worker-outbox-events.example.workers.dev",
    })
