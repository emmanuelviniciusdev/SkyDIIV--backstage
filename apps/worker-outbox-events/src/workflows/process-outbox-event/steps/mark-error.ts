...
export async function markOutboxEventErrorStep(outboxEventId: string): Promise<void> {
  const log = createLogger("mark-error")
  log.info("Step started", { outboxEventId })

  const db = getDb()
  const repo = new SqlOutboxEventsRepository(db)
  await repo.updateStatus(outboxEventId, "ERROR")
