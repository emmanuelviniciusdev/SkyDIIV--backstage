...
describe("worker fetch routing", () => {
  ...
  it("returns 401 for an unsigned email--welcome request", async () => {
    mockWorkflowsFetch.mockResolvedValueOnce(new Response("Unauthorized", { status: 401 }))
    const res = await worker.fetch(makeRequest("POST", "/email--welcome"), {}, makeCtx())
  ...
  it("propagates errors thrown by the workflow router", async () => {
    ...
    await expect(
      worker.fetch(makeRequest("POST", "/email--welcome"), {}, makeCtx()),
