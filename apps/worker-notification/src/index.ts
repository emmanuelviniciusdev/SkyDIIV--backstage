...
const SERVICE_NAME = "worker-notification"

/**
 * Cloudflare Worker entry point — worker-notification.
 *
 * A generic host for SkyDIIV notification Upstash Workflows. Copies all Worker
 * bindings into process.env so downstream modules that read process.env work
 * without modification, then delegates to the @upstash/workflow serveMany router.
 *
 * GET / → health-check (useful for uptime monitors).
 * All other paths → serveMany, which dispatches by the last path segment:
 *   POST /email--welcome → email--welcome workflow
 */
export default {
