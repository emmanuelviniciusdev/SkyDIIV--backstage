...
export function buildWordmarkHtml(fontFamily: string): string {
  const letters = LOGO_LETTER_GRADIENTS.map(({ ch, top, bottom }) => {
    const glyph = ch === "i" ? "i" : ch
    return `<span style="display:inline-block;color:${top};background:linear-gradient(180deg,${top} 0%,${bottom} 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;font-family:${fontFamily};font-weight:300;">${glyph}</span>`
  }).join("")
