# Internationalization (i18n)

This worker generates user-facing text (LLM prompts, weather summaries, wardrobe panorama markdown, marketplace search terms) in the user's preferred language. Generate-search-terms uses a marketplace language instead when the catalog does not list the user's locale.

---

## Supported locales

| Code | Language | Default |
|---|---|---|
| `pt-BR` | Português (Brasil) | Yes |
| `es-PE` | Español (Perú) | |
| `en-US` | English (United States) | |

These codes align with the SkyDIIV web app (`web/lib/i18n/config.ts`) and the `domains` table seeded with type `language`.

---

## How the user's language is resolved

Workflows call `resolveUserLocale(userId)` when loading context or building a prompt. The lookup reads `app_preferences.language_id` joined to `domains.name` and maps the domain name to a locale code.

```mermaid
flowchart TD
    START["build-prompt step"] --> RESOLVE["resolveUserLocale"]
    RESOLVE --> QUERY["SELECT domains.name<br/>FROM app_preferences<br/>JOIN domains ON language_id"]
    QUERY --> HAS_PREFS{"app_preferences<br/>row exists?"}
    HAS_PREFS -->|No| DEFAULT["Fallback: pt-BR"]
    HAS_PREFS -->|Yes| MAP{"domains.name"}
    MAP -->|"Português BR"| PT["pt-BR"]
    MAP -->|"Español PE"| ES["es-PE"]
    MAP -->|"English US"| EN["en-US"]
    MAP -->|Unknown| DEFAULT
    PT --> BUILD["Build prompt and user-facing strings"]
    ES --> BUILD
    EN --> BUILD
    DEFAULT --> BUILD
```

### Database model

```mermaid
erDiagram
    users ||--o| app_preferences : "has"
    domains ||--o{ app_preferences : "language_id"

    users {
        uuid id PK
    }

    app_preferences {
        uuid id PK
        uuid user_id FK
        uuid language_id FK
    }

    domains {
        uuid id PK
        string name "Português BR, Español PE, English US"
        string type "language"
        string source "always null for languages"
    }
```

| Table | Column | Purpose |
|---|---|---|
| `app_preferences` | `language_id` | FK to the user's language domain |
| `domains` | `name`, `type` | Language lookup (`type = 'language'`) |

---

## Module layout

```mermaid
flowchart TB
    subgraph entry["Workflow steps"]
        WBP["generate-weekly-outfits<br/>build-prompt.ts"]
        PBP["generate-wardrobe-panorama<br/>build-prompt.ts"]
        SBP["generate-search-terms<br/>build-prompt.ts"]
    end

    subgraph i18n["src/lib/i18n"]
        RESOLVE["resolve-user-locale.ts"]
        REPO["app-preferences.repository.ts"]
        CONFIG["config.ts"]

        subgraph locales["locales/"]
            PT["pt-BR.ts"]
            ES["es-PE.ts"]
            EN["en-US.ts"]
        end

        subgraph prompts["prompts/"]
            WEEKLY["weekly-outfits.ts"]
            PANORAMA["wardrobe-panorama.ts"]
            SEARCH["generate-search-terms.ts"]
            SUM["summarize-automatic-thrifting-feedback.ts"]
        end

        subgraph weather["weather/"]
            CODES["weather-codes.ts"]
            FMT["formatters.ts"]
        end
    end

    LEGACY["prompt/builder.ts<br/>delegates + parseOutfitSuggestions"]

    WBP --> RESOLVE
    PBP --> RESOLVE
    SBP --> RESOLVE
    RESOLVE --> REPO
    RESOLVE --> CONFIG
    WBP --> LEGACY
    LEGACY --> WEEKLY
    LEGACY --> FMT
    PBP --> PANORAMA
    SBP --> SEARCH
    WEEKLY --> locales
    WEEKLY --> FMT
    PANORAMA --> locales
    SEARCH --> CONFIG
    FMT --> locales
    FMT --> CODES
```

The legacy `src/lib/prompt/builder.ts` delegates to this module and remains the public API for weekly-outfits parsing (`parseOutfitSuggestions`).

---

## What is localized per workflow

```mermaid
flowchart LR
    LOCALE["resolveUserLocale"]

    subgraph weekly["generate-weekly-outfits"]
        W1["build-prompt"]
        W2["LLM prompt<br/>instructions + labels"]
        W3["weekly_outfits weather fields"]
        W4["LLM JSON keys<br/>sunday…saturday"]
        W1 --> W2
        W1 --> W3
        W2 --> W4
    end

    subgraph panorama["generate-wardrobe-panorama"]
        P1["build-prompt"]
        P2["LLM prompt<br/>sections + labels"]
        P3["wardrobe_panorama.content<br/>markdown"]
        P1 --> P2
        P2 --> P3
    end

    subgraph search["generate-search-terms"]
        S1["load-context"]
        S2["resolveSearchTermsLocale"]
        S3["LLM prompt<br/>term language"]
        S1 --> S2
        S2 --> S3
    end

    LOCALE --> W1
    LOCALE --> P1
    LOCALE --> S1

    style W4 fill:#f9f9f9,stroke:#999
```

Legend: shaded nodes are **not** localized (schema-stable English keys).

### generate-weekly-outfits

| Output | Localized? | Notes |
|---|---|---|
| LLM prompt (instructions + data labels) | Yes | Template selected by locale |
| `weekly_outfits.weather_summary` | Yes | Stored in the user's locale |
| `weekly_outfits.weather_code` | No | WMO code from Open-Meteo (locale-independent integer) |
| `weekly_outfits.description_temperature` | Yes | WMO weather code description in the user's locale |
| `weekly_outfits.min_temperature` / `max_temperature` | No | Raw Celsius values from Open-Meteo |
| `weekly_outfits.unity_temperature` | No | Always `°C` (matches the weather provider) |
| LLM JSON weekday keys (`sunday`, …) | No | Schema-stable English keys |

### generate-wardrobe-panorama

| Output | Localized? | Notes |
|---|---|---|
| LLM prompt (instructions + section headers) | Yes | Built by `buildWardrobePanoramaPrompt()` |
| `wardrobe_panorama.content` | Yes | LLM output language follows the prompt locale |

### generate-search-terms-products-scraping

Search terms follow the user's locale when that locale is listed in the marketplace `supported_languages`. Otherwise they follow a language from `supported_languages`. The optional feedback-summary block and the summarizer text use that same resolved locale (`resolveSearchTermsLocale()`).

See [GENERATE_SEARCH_TERMS_PRODUCTS_SCRAPING.md](./GENERATE_SEARCH_TERMS_PRODUCTS_SCRAPING.md) for trigger, factories, and skip rules.

```mermaid
flowchart TD
    USER["User locale"] --> MATCH{"Marketplace lists<br/>the user locale?"}
    CATALOG["supported_languages"] --> MATCH
    MATCH -->|Yes| USERLANG["Generate terms in the user locale"]
    MATCH -->|No| FALLBACK["Generate terms in a marketplace language"]
```

| Output | Localized? | Notes |
|---|---|---|
| LLM prompt output-language line | Yes | `resolveSearchTermsLocale()` — user locale if listed, else a marketplace language |
| `json_search.term` | Yes | Written in that resolved locale |
| Feedback summary (`summaries_feedbacks_automatic_thrifting.summary`) | Yes | Same locale; optional HISTÓRICO DE FEEDBACK block on the search-term prompt |
| Summarizer prompt | Yes | Tendencies, not exclusive wants |

---

## Adding a new locale

```mermaid
flowchart TD
    A["1. Add locale to LOCALES<br/>and DOMAIN_NAME_TO_CODE in config.ts"] --> B["2. Create locales/&lt;code&gt;.ts"]
    B --> C["3. Register in locales/index.ts"]
    C --> D["4. Add prompt templates<br/>in prompts/weekly-outfits.ts<br/>and prompts/wardrobe-panorama.ts"]
    D --> E["5. Add tests under tests/unit/i18n/"]
```

---

## Tests

| File | Coverage |
|---|---|
| `tests/unit/i18n/config.test.ts` | Locale resolution and domain mapping |
| `tests/unit/i18n/app-preferences-repository.test.ts` | SQL repository |
| `tests/unit/i18n/resolve-user-locale.test.ts` | End-to-end locale lookup |
| `tests/unit/i18n/weather-formatters.test.ts` | Per-locale weather strings |
| `tests/unit/i18n/weekly-outfits-prompt.test.ts` | Prompt templates (3 locales) |
| `tests/unit/i18n/wardrobe-panorama-prompt.test.ts` | Panorama prompts (3 locales) |
| `tests/unit/i18n/generate-search-terms-prompt.test.ts` | Search-term prompt output language, optional feedback summary, variety instruction |
| `tests/unit/generate-search-terms.test.ts` | Marketplace eligibility and `resolveSearchTermsLocale` |

Integration tests in `tests/integration/` mock `app_preferences` queries and assert default `pt-BR` resolution. Generate-search-terms integration tests cover user-locale vs marketplace-language matching.
